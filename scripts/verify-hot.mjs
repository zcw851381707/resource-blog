...
const r = await c.execute(`SELECT title, clickCount FROM Drama ORDER BY clickCount DESC LIMIT 10`);
console.log('Turso Top 10:');
r.rows.forEach(row => console.log(`  ${row[0]}: ${row[1]}`));
await c.close();
